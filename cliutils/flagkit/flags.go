...
var flagsMap = map[string]components.Flag{
	...
	specVars:          components.NewStringFlag(specVars, "List of semicolon-separated(;) variables in the form of \"key1=value1;key2=value2;...\" (wrapped by quotes) to be replaced in the File Spec. In the File Spec, the variables should be used as follows: ${key1}.", components.SetMandatoryFalse()),
	...
	module:            components.NewStringFlag(module, "Optional module name for the build-info. Build name and number options are mandatory when this option is provided.", components.SetMandatoryFalse()),
	...
	dryRun:            components.NewBoolFlag(dryRun, "Set to true to disable communication with Artifactory.", components.WithBoolDefaultValueFalse()),
	...
	failNoOp:          components.NewBoolFlag(failNoOp, "Set to true if you'd like the command to return exit code 2 in case of no files are affected.", components.WithBoolDefaultValueFalse()),
	...
	bundle:            components.NewStringFlag(bundle, "If specified, only artifacts of the specified bundle are matched. The value format is bundle-name/bundle-version.", components.SetMandatoryFalse()),
	...
	configPlatformUrl: components.NewStringFlag(url, "JFrog platform URL. (example: https://acme.jfrog.io)", components.SetMandatoryFalse()),
	configRtUrl:       components.NewStringFlag(configRtUrl, "JFrog Artifactory URL. (example: https://acme.jfrog.io/artifactory)", components.SetMandatoryFalse()),
	configDistUrl:     components.NewStringFlag(configDistUrl, "JFrog Distribution URL. (example: https://acme.jfrog.io/distribution)", components.SetMandatoryFalse()),
	configXrUrl:       components.NewStringFlag(configXrUrl, "JFrog Xray URL. (example: https://acme.jfrog.io/xray)", components.SetMandatoryFalse()),
	configMcUrl:       components.NewStringFlag(configMcUrl, "JFrog Mission Control URL. (example: https://acme.jfrog.io/mc)", components.SetMandatoryFalse()),
	configPlUrl:       components.NewStringFlag(configPlUrl, "JFrog Pipelines URL. (example: https://acme.jfrog.io/pipelines)", components.SetMandatoryFalse()),
	configUser:        components.NewStringFlag(user, "JFrog Platform username.", components.SetMandatoryFalse()),
	configPassword:    components.NewStringFlag(password, "JFrog Platform password or API key.", components.SetMandatoryFalse()),
	configAccessToken: components.NewStringFlag(accessToken, "JFrog Platform access token.", components.SetMandatoryFalse()),
	BasicAuthOnly:     components.NewBoolFlag(BasicAuthOnly, "Set to true to disable replacing username and password/API key with an automatically created access token that's refreshed hourly. Username and password/API key will still be used with commands which use external tools or the JFrog Distribution service. Can only be passed along with username and password/API key options.", components.WithBoolDefaultValueFalse()),
	configInsecureTls: components.NewBoolFlag(InsecureTls, "Set to true to skip TLS certificates verification, while encrypting the Artifactory password during the config process.", components.WithBoolDefaultValueFalse()),
	Overwrite:         components.NewBoolFlag(Overwrite, "Overwrites the instance configuration if an instance with the same ID already exists.", components.WithBoolDefaultValueFalse()),
	passwordStdin:     components.NewBoolFlag(passwordStdin, "Set to true if you'd like to provide the password via stdin.", components.WithBoolDefaultValueFalse()),
	accessTokenStdin:  components.NewBoolFlag(accessTokenStdin, "Set to true if you'd like to provide the access token via stdin.", components.WithBoolDefaultValueFalse()),
	...
	limit:                   components.NewStringFlag(limit, "The maximum number of items to fetch. Usually used with the 'sort-by' option.", components.SetMandatoryFalse()),
	offset:                  components.NewStringFlag(offset, "The offset from which to fetch items (i.e. how many items should be skipped). Usually used with the 'sort-by' option.", components.SetMandatoryFalse()),
	...
	publicGpgKey:            components.NewStringFlag(publicGpgKey, "Path to the public GPG key file located on the file system, used to validate downloaded release bundles.", components.SetMandatoryFalse()),
	...
	downloadProps:           components.NewStringFlag(props, "List of semicolon-separated(;) properties in the form of \"key1=value1;key2=value2;...\". Only artifacts with these properties will be downloaded.", components.SetMandatoryFalse()),
	downloadExcludeProps:    components.NewStringFlag(excludeProps, "List of semicolon-separated(;) properties in the form of \"key1=value1;key2=value2;...\". Only artifacts without the specified properties will be downloaded.", components.SetMandatoryFalse()),
	archiveEntries:          components.NewStringFlag(archiveEntries, "This option is no longer supported since version 7.90.5 of Artifactory. If specified, only archive artifacts containing entries matching this pattern are matched. You can use wildcards to specify multiple artifacts.", components.SetMandatoryFalse()),
	downloadSyncDeletes:     components.NewStringFlag(syncDeletes, "Specific path in the local file system, under which to sync dependencies after the download. After the download, this path will include only the dependencies downloaded during this download operation. The other files under this path will be deleted.", components.SetMandatoryFalse()),
	...
	uploadTargetProps: components.NewStringFlag(targetProps, "List of semicolon-separated(;) properties in the form of \"key1=value1;key2=value2;...\". Those properties will be attached to the uploaded artifacts.", components.SetMandatoryFalse()),
	uploadExclusions:  components.NewStringFlag(exclusions, "List of semicolon-separated(;) exclude patterns. Exclude patterns may contain the * and the ? wildcards or a regex pattern, according to the value of the 'regexp' option.", components.SetMandatoryFalse()),
	deb:               components.NewStringFlag(deb, "Used for Debian packages in the form of distribution/component/architecture. If the value for distribution, component or architecture includes a slash, the slash should be escaped with a back-slash.", components.SetMandatoryFalse()),
	...
	uploadFlat:        components.NewBoolFlag(flat, "If set to false, files are uploaded according to their file system hierarchy.", components.WithBoolDefaultValueFalse()),
	uploadRegexp:      components.NewBoolFlag(regexpFlag, "Set to true to use a regular expression instead of wildcards expression to collect files to upload.", components.WithBoolDefaultValueFalse()),
	uploadExplode:     components.NewBoolFlag(explode, "Set to true to extract an archive after it is deployed to Artifactory.", components.WithBoolDefaultValueFalse()),
	symlinks:          components.NewBoolFlag(symlinks, "Set to true to preserve symbolic links structure in Artifactory.", components.WithBoolDefaultValueFalse()),
	uploadSyncDeletes: components.NewStringFlag(syncDeletes, "Specific path in Artifactory, under which to sync artifacts after the upload. After the upload, this path will include only the artifacts uploaded during this upload operation. The other files under this path will be deleted.", components.SetMandatoryFalse()),
	uploadAnt:         components.NewBoolFlag(antFlag, "Set to true to use an ant pattern instead of wildcards expression to collect files to upload.", components.WithBoolDefaultValueFalse()),
	uploadArchive:     components.NewStringFlag(archive, "Set to \"zip\" to pack and deploy the files to Artifactory inside a ZIP archive. Currently, the only packaging format supported is zip.", components.SetMandatoryFalse()),
	...
	searchExcludeProps: components.NewStringFlag(excludeProps, "List of semicolon-separated(;) properties in the form of \"key1=value1;key2=value2;...\". Only artifacts without the specified properties will be returned.", components.SetMandatoryFalse()),
	searchTransitive:   components.NewBoolFlag(transitive, "Set to true to look for artifacts also in remote repositories. The search will run on the first five remote repositories within the virtual repository. Available on Artifactory version 7.17.0 or higher.", components.WithBoolDefaultValueFalse()),
	searchInclude:      components.NewStringFlag(searchInclude, "List of semicolon-separated(;) fields in the form of \"value1;value2;...\". Only the path and the fields that are specified will be returned. The fields must be part of the 'items' AQL domain. For the full supported items list, check %sjfrog-artifactory-documentation/artifactory-query-language.", components.SetMandatoryFalse()),
	...
	badRegexp:    components.NewBoolFlag(regexpFlag, "Set to true to use a regular expression instead of wildcards expression to collect files to be added to the build info."),
	...
	fail: components.NewBoolFlag(fail, "Set to true if you'd like the command to return exit code 2 in case of no files are affected.", components.WithBoolDefaultValueFalse()),

	// BuildPromote specific commands flags
	Status:              components.NewStringFlag(Status, "Build promotion status.", components.SetMandatoryFalse()),
	comment:             components.NewStringFlag(comment, "Build promotion comment.", components.SetMandatoryFalse()),
	sourceRepo:          components.NewStringFlag(sourceRepo, "Build promotion source repository.", components.SetMandatoryFalse()),
	includeDependencies: components.NewBoolFlag(includeDependencies, "If true, the build dependencies are also promoted.", components.WithBoolDefaultValueFalse()),
	copyFlag:            components.NewBoolFlag(copyFlag, "If true, the build artifacts and dependencies are copied to the target repository, otherwise they are moved.", components.WithBoolDefaultValueFalse()),
	...
	maxBuilds:       components.NewStringFlag(maxBuilds, "The maximum number of builds to store in Artifactory.", components.SetMandatoryFalse()),
	...
	serverIdResolve: components.NewStringFlag(serverIdResolve, "Artifactory server ID for resolution. The server should be configured using the 'jfrog c add' command.", components.SetMandatoryFalse()),
	repoResolve:     components.NewStringFlag(repoResolve, "Repository for dependencies resolution.", components.SetMandatoryFalse()),

	// MvnConfig specific commands flags
	serverIdDeploy:       components.NewStringFlag(serverIdDeploy, "Artifactory server ID for deployment. The server should be configured using the 'jfrog c add' command.", components.SetMandatoryFalse()),
	repoResolveReleases:  components.NewStringFlag(repoResolveReleases, "Resolution repository for release dependencies.", components.SetMandatoryFalse()),
	repoResolveSnapshots: components.NewStringFlag(repoResolveSnapshots, "Resolution repository for snapshot dependencies.", components.SetMandatoryFalse()),
	repoDeployReleases:   components.NewStringFlag(repoDeployReleases, "Deployment repository for release artifacts.", components.SetMandatoryFalse()),
	repoDeploySnapshots:  components.NewStringFlag(repoDeploySnapshots, "Deployment repository for snapshot artifacts.", components.SetMandatoryFalse()),
	includePatterns:      components.NewStringFlag(includePatterns, "Filter deployed artifacts by setting a wildcard pattern that specifies which artifacts to include. You may provide multiple patterns separated by ', '.", components.SetMandatoryFalse()),
	excludePatterns:      components.NewStringFlag(excludePatterns, "Filter deployed artifacts by setting a wildcard pattern that specifies which artifacts to exclude. You may provide multiple patterns separated by ', '.", components.SetMandatoryFalse()),
	UseWrapper:           components.NewBoolFlag(UseWrapper, "Set to true if you wish to use the wrapper.", components.WithBoolDefaultValueFalse()),

	// GradleConfig specific commands flags
	repoDeploy:          components.NewStringFlag(repoDeploy, "Repository for artifacts deployment.", components.SetMandatoryFalse()),
	usesPlugin:          components.NewBoolFlag(usesPlugin, "Set to true if the Gradle Artifactory Plugin is already applied in the build script.", components.WithBoolDefaultValueFalse()),
	...
	xrayScan:          components.NewBoolFlag(xrayScan, "Set if you'd like all files to be scanned by Xray on the local file system prior to the upload, and skip the upload if any of the files are found vulnerable.", components.WithBoolDefaultValueFalse()),
	...
	skipLogin:           components.NewBoolFlag(skipLogin, "Set to true if you'd like the command to skip performing docker login.", components.WithBoolDefaultValueFalse()),
	validateSha:         components.NewBoolFlag(validateSha, "Set to true to enable SHA validation during Docker push.", components.WithBoolDefaultValueFalse()),
	watches:             components.NewStringFlag(watches, "A comma-separated(,) list of Xray watches, to determine Xray's violations creation.", components.SetMandatoryFalse()),
	repoPath:            components.NewStringFlag(repoPath, "Target repo path, to enable Xray to determine watches accordingly.", components.SetMandatoryFalse()),
	licenses:            components.NewBoolFlag(licenses, "Set to true if you'd like to receive licenses from Xray scanning.", components.WithBoolDefaultValueFalse()),
	ExtendedTable:       components.NewBoolFlag(ExtendedTable, "Set to true if you'd like the table to include extended fields such as 'CVSS' & 'Xray Issue Id'. Ignored if the 'format' provided is not 'table'.", components.WithBoolDefaultValueFalse()),
	BypassArchiveLimits: components.NewBoolFlag(BypassArchiveLimits, "Set to true to bypass the indexer-app archive limits.", components.WithBoolDefaultValueFalse()),
	MinSeverity:         components.NewStringFlag(MinSeverity, "Set the minimum severity of issues to display. The following values are accepted: Low, Medium, High or Critical.", components.SetMandatoryFalse()),
	FixableOnly:         components.NewBoolFlag(FixableOnly, "Set to true if you wish to display issues that have a fixed version only.", components.WithBoolDefaultValueFalse()),
	vuln:                components.NewBoolFlag(vuln, "Set to true if you'd like to receive an additional view of all vulnerabilities, regardless of the policy configured in Xray. Ignored if the provided 'format' is 'sarif'.", components.WithBoolDefaultValueFalse()),

	// DockerPromote specific commands flags
	targetDockerImage: components.NewStringFlag("target-docker-image", "Docker target image name.", components.SetMandatoryFalse()),
	...
	noFallback:          components.NewBoolFlag(noFallback, "Set to true to avoid downloading packages from the VCS, if they are missing in Artifactory.", components.WithBoolDefaultValueFalse()),
	...
	IncludeProjects: components.NewStringFlag(IncludeProjects, "List of semicolon-separated(;) JFrog Project keys to include in the transfer. You can use wildcards to specify patterns for the JFrog Project keys.", components.SetMandatoryFalse()),
	...
	rtAtcGrantAdmin:  components.NewBoolFlag(GrantAdmin, "Set to true to provide admin privileges to the access token. This is only available for administrators.", components.WithBoolDefaultValueFalse()),
	...
	rtAtcRefreshable: components.NewBoolFlag(Refreshable, "Set to true if you'd like the token to be refreshable. A refresh token will also be returned in order to be used to generate a new token once it expires.", components.WithBoolDefaultValueFalse()),
	rtAtcAudience:    components.NewStringFlag(Audience, "A space-separated list of the other Artifactory instances or services that should accept this token identified by their Artifactory Service IDs, as obtained by the 'jfrog rt curl api/system/service_id' command.", components.SetMandatoryFalse()),

	// UserCreate
	Admin:          components.NewBoolFlag(Admin, "Set to true if you'd like to create an admin user.", components.WithBoolDefaultValueFalse()),
	...
	UsersGroups:    components.NewStringFlag(UsersGroups, "A list of comma-separated(,) groups for the new users to be associated with.` `", components.SetMandatoryFalse()),

	// UsersDelete
	usersDeleteCsv: components.NewStringFlag(csv, "Path to a CSV file with the users' details. The first row of the file is reserved for the cells' headers. It must include \"username\"", components.SetMandatoryFalse()),

	// GroupCreate
	Replace: components.NewBoolFlag(Replace, "Set to true if you'd like existing groups to be replaced.", components.WithBoolDefaultValueFalse()),
